...
#벨만 - 포드 알고리즘
def bf(start):
    dist[start] = 0 # 시작 노드에 대해서 거리 0으로 초기화
    for i in range(n): # 정점 수만큼 반복
        for j in range(m): #매 반복마다 모든 간선 확인
            node = edges[j][0] #현재 노드 받아오기
            adj_node = edges[j][1] #인접한 다음 노드 받아오기
            cost = edges[j][2] #가중치 받아오기
            #현재 간선을 통해서 다른 노드로 이동하는 거리가 더 기존보다 더 짧은 경우
            if dist[node] != INF and dist[node]+cost < dist[adj_node]:
