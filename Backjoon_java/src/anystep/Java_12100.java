...
//2.2 이동방향이 위와 왼쪽은 좌측상단부터 탐색 시작, 이동방향이 아래와 오른쪽은 우측하단부터 탐색 시작
//2.3 블록이 이동할 좌표값이 공백이면 경계를 벗어날때까지 계속 이동
//2.4 블록이 이동할 좌표가 같은 값을 지닌 블록이면 합체여부에 따라 합체종료 혹은 바로종료
//2.5 블록이 이동할 좌표가 다른 값을 지닌 블록이면 바로 종료
//3. 최대깊이까지 재귀함수가 도달하면 현재 지니고 있는 맵의 최대값을 탐색 후 최대값 비교 갱신
//4. 정답값 출력
//풀이시간 1시간 27분 12초
